...
TinyMCE.prototype.init = function(settings) {
	...
	this.defParam("visual_table_class", "border: 1px dashed #BBBBBB");
...
TinyMCE.prototype._cleanupElementName = function(element_name, element) {
	...
	/*if (element_name == "noscript")
	{
		...
		elmData.element_name = "p";
		elmData.valid_attribs = element.setAttribute("id", "noscript");;
		//elmData.innerHTML=element_name.innerHTML;
		return elmData;
	}*/
	//Edited by Naveen
...
//		case "className":
		case "class":
			...
			if (element_name == "table") {   //Edited by Naveen
				// Handle visual aid
...
TinyMCE.prototype.cleanupNode = function(node) {
	...
	switch (node.nodeType) {
		case 1: // Element
			...
			// Is MSIE noscript element
			if (tinyMCE.isMSIE && elementName == "noscript") {
				//alert(node.text);
				if (node.hasChildNodes()) {
					for (var i=0; i<node.childNodes.length; i++)
						output += this.cleanupNode(node.childNodes[i]);
		...
		case 3: // Text
			...
			//if (node.parentNode.nodeName.toLowerCase() == "noscript")
			//	return node.nodeValue;
...
TinyMCE.prototype.handleVisualAid = function(element, deep, state) {
	...
	switch (element.nodeName.toLowerCase()) {
		case "table":
			var cssText = element.getAttribute("border") == 0 ? tinyMCE.settings['visual_table_class'] : "";
...
TinyMCEControl.prototype.execCommand = function(command, user_interface, value) {
	...
	switch (command) {
		case "mceStoreSelection":
			...
			break;
		case 'mceFlash':
			var attrib='width=450,height=200,directories=no,location=no,menubar=no,status=no,top=277,left=370';
			attrib+=',titlebar=no,toolbar=no';
			attrib+=',scrollbars=no,resizeable=no';
			window.open("jscripts/tiny_mce/plugins/flash/flash.aspx","mceFlash",attrib);
		return;
		...
		case "mceSetCSSClass":
			...
			if (selectedText && !tinyMCE.selectedNode) {
				...
				if (value == null)
				{	//Edited by Naveen
					if (tinyMCE.selectedElement.nodeName.toLowerCase() == "span"){      //Edited by Naveen
						targetNode = tinyMCE.selectedElement;	                    //Edited by Naveen
						targetNode.removeAttribute("className");	            //Edited by Naveen
						targetNode.removeAttribute("class");	                    //Edited by Naveen
						this.execCommand("mceRemoveNode", false, targetNode);       //Edited by Naveen  
					}        //Edited by Naveen
						
					//var spanElm = this.getDoc().removeElement("span");
					//spanElm.className = "";
					//alert("hello");
