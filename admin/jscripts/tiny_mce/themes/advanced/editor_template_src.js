...
var TinyMCE_advanced_buttons = [
	...
	['advdoc', 'newdocument.gif', '{$lang_newdocument_desc}', 'mceNewDocument']
];
...
function TinyMCE_advanced_execCommand(editor_id, element, command, user_interface, value)
{
	switch (command)
	{
		...
		case "mceNewDocument": //Edited For Upload document by Naveen Bansal
			var template = new Array();
			
			template['file']='../../plugins/advdoc/doc.php';
			template['width']=410;
			template['height']=280;
			tinyMCE.openWindow(template,{editor_id:editor_id});
		return true;
	...
	if (pathElm)
	{
		...
		for (var i=path.length-1; i>=0; i--)
		{
			...
			if (getAttrib(path[i], 'name').indexOf("mce_") != 0)
			{
				if (getAttrib(path[i], "className") != "")
				{
					//getAttrib(path[i], "className").substr(0,getAttrib(path[i], "className").indexOf("border"));
					//nodeName += "." + getAttrib(path[i], "className");
					if(getAttrib(path[i], "className").indexOf("border")>0){
						nodeName += "." + getAttrib(path[i], "className").substr(0,getAttrib(path[i], "className").indexOf("border"));
						nodeData = "class: " + getAttrib(path[i], "className").substr(0,getAttrib(path[i], "className").indexOf("border"));
					}
					else
						nodeData="";
   ...
   tinyMCE.switchClassSticky(editor_id + '_hr', 'mceButtonNormal');
	...
	if (selectElm)
	{
		...
		else
		{
			...
			selectByValue(selectElm, "");  //Edited by Naveen
		}
	}
